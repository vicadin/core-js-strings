...
function findLongestWord(sentence) {
  ...
  let countLettersLongestWord = 0;
  let longestWord = '';
  for (let i = 0; i < sentenceWords.length - 1; i += 1) {
    if (sentenceWords[i].length > countLettersLongestWord) {
      longestWord = sentenceWords[i];
      countLettersLongestWord = sentenceWords[i].length;
    }
  }
...
function reverseWords(str) {
  ...
  const strArrReverse = strArr.map((word) => word.split('').reverse().join(''));
...
function invertCase(str) {
  const strArr = str.split('');
  const strArrReverseCase = strArr.map((letter) => {
    if (letter === letter.toLowerCase()) return letter.toUpperCase();
    return letter.toLowerCase();
  });
  return strArrReverseCase.join('');
}
...
function encodeToRot13(str) {
  ...
  let encodeToRot13Str = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] !== ' ' && str[i] !== '?' && str[i] !== '!') {
      const letterIndex = alphabet.indexOf(str[i]);
      encodeToRot13Str += alphabetRot13[letterIndex];
    } else encodeToRot13Str += str[i];
  }
  return encodeToRot13Str;
}
